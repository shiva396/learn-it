import 'package:flutter/material.dart';

class LColors {
  // Primary theme colors
  static const Color blue = Color(0xFF0A84FF);
  static const Color blueLight = Color(0xFF64B5F6);
  static const Color blueDark = Color(0xFF0050C8);

  // Secondary and background
  static const Color background = Color(0xFFF5F9FF);
  static const Color surface = Color(0xFFEAF1FB);

  // Greys for text and dividers
  static const Color greyWhite = Color(0xFFFAFBFC);
  static const Color grey = Color(0xFF9CA3AF);
  static const Color greyLight = Color(0xFFD1D5DB);
  static const Color greyDark = Color(0xFF4B5563);

  // Utility colors
  static const Color black = Color(0xFF111827);
  static const Color error = Color(0xFFEF4444);
  static const Color success = Color(0xFF10B981);
  static const Color warning = Color(0xFFF59E0B);

  // Educational Highlight (optional accent)
  static const Color highlight = Color(0xFF7C3AED);

  //Icon Colors
  static const Color iconColor = Color(0xFFEFEBE0);

  //Button Colors
  static const buttonDarkBackgroundColor = Color.fromARGB(255, 0, 27, 90);
  static const Color buttonDarkColor = Color.fromARGB(255, 45, 115, 176);
}
